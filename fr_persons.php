    ...
    <div class="container">
		...
		<div class="row">
			...
			<table class="table table-striped table-bordered" style="background-color: lightgrey !important">
				...
				<tbody>
					<?php 
						include '../database/database_fr.php';
						...
						foreach ($pdo->query($sql) as $row) {
							...
							if ($row['countAssigns'] == 0)
								echo '<td>'. trim(htmlspecialchars($row['lname'])) . ', ' . trim(htmlspecialchars($row['fname'])) . ' (' . substr(htmlspecialchars($row['title']), 0, 1) . ') '.' - UNASSIGNED</td>';
							else
								echo '<td>'. trim(htmlspecialchars($row['lname'])) . ', ' . trim(htmlspecialchars($row['fname'])) . ' (' . substr(htmlspecialchars($row['title']), 0, 1) . ') - '.$row['countAssigns']. ' events</td>';
