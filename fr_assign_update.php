					...
					<div class="controls">
						<?php
							...
							foreach ($pdo->query($sql) as $row) {
								if($row['id']==$person)
									echo "<option selected value='" . $row['id'] . " '> " . htmlspecialchars($row['lname']) . ', ' .htmlspecialchars($row['fname']) . "</option>";
								else
									echo "<option value='" . $row['id'] . " '> " . htmlspecialchars($row['lname']) . ', ' .htmlspecialchars($row['fname']) . "</option>";
							}
							echo "</select>";
							Database::disconnect();
						?>
				...
				<div class="control-group">
					...
					<div class="controls">
						<?php
							...
							foreach ($pdo->query($sql) as $row) {
								if($row['id']==$event) {
									echo "<option selected value='" . $row['id'] . " '> " . Functions::dayMonthDate($row['event_date']) . " (" . Functions::timeAmPm($row['event_time']) . ") - " . trim(htmlspecialchars($row['event_description'])) . " (" . trim(htmlspecialchars($row['event_location'])) . ") " . "</option>";
								}
								else {
									echo "<option value='" . $row['id'] . " '> " . Functions::dayMonthDate($row['event_date']) . " (" . Functions::timeAmPm($row['event_time']) . ") - " . trim(htmlspecialchars($row['event_description'])) . " (" . trim(htmlspecialchars($row['event_location'])) . ") " . "</option>";
								}
							}
							echo "</select>";
							Database::disconnect();
